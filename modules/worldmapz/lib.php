	...
	if (get_module_setting("usestamina") == 1){
		...
		switch ($terrain['type']){
			...
			case "Snow":
				...
				break;
			case "Cave":
				return stamina_getdisplaycost("Travelling - Cave",2);
				break;
			case "Road":
				return stamina_getdisplaycost("Travelling - Road",2);
				break;
			case "Air":
...
function worldmapen_terrain_takestamina($x, $y, $z=1) {
	...
	switch ($terrain['type']){
		...
		case "Snow":
			...
			break;
		case "Cave":
			$cave = process_action("Travelling - Cave");
			break;
		case "Road":
			$road = process_action("Travelling - Road");
			break;
		case "Air":
	...
	if ($cave['lvlinfo']['levelledup']==true){
		output("`n`c`b`0You gained a level in Travel across caves!  You are now level %s!  This action will cost fewer Stamina points now, so you can traverse more caves in a single day.`b`c`n",$snow['lvlinfo']['newlvl']);
	}
	if ($road['lvlinfo']['levelledup']==true){
		output("`n`c`b`0You gained a level in Travel across roads!  You are now level %s!  This action will cost fewer Stamina points now, so you can zip down more roads in a single day.`b`c`n",$snow['lvlinfo']['newlvl']);
	}
	if ($air['lvlinfo']['levelledup']==true){
		output("`n`c`b`0You gained a level in Travel across air!  You are now level %s!  This action will cost fewer Stamina points now, so you can fly further in a single day.`b`c`n",$snow['lvlinfo']['newlvl']);
	}
}
	...
	if ($id!=0){
		switch ($terrain['type']){
			...
			case "Snow":
				...
				break;
			case "Cave":
				$terrain['encounter']=$terrain['encounter']*get_module_objpref("mounts",$id,"encounterCave");
				break;
			case "Road":
				$terrain['encounter']=$terrain['encounter']*get_module_objpref("mounts",$id,"encounterRoad");
			case "Air":
...
//	debug("x={$x}, y={$y}, z={$z}, type={$type}");
	...
	if (is_numeric($type)) {
		switch ($type) {
			...
			case 8:		$type = "Cave";  break;
			case 9:		$type = "Air";	  break;
			case 10:	$type = "Road";	break;
...
function worldmapen_loadTerrainDefs() {
	...
	if ( ( !isset( $worldmapen_globals["terrainDefs"] ) ) || is_null($worldmapen_globals["terrainDefs"])) {
		$worldmapen_globals["terrainDefs"] = array(
			...
			"Earth"	 => array("type" => "Cave",  "color" => get_module_setting("colorCave"),  "moveCost" => get_module_setting("moveCostCave"),  "encounter" => get_module_setting("encounterCave")),
			"Road"	 => array("type" => "Road",  "color" => get_module_setting("colorRoad"),  "moveCost" => get_module_setting("moveCostRoad"),  "encounter" => get_module_setting("encounterRoad"))
